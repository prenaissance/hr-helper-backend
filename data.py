...
class Data:
    def __init__(self, initial_path=None, formated_path=None) -> None:
        ...
        if formated_path is None and initial_path is not None:
            self.data = Format.get_formated_data(Format.load_json(initial_path) ,sorted_time=True)
        elif formated_path is not None:
            self.data = Format.load_json(formated_path)
        # if path2 and path1 are both none a default path1 will be provided
            ...
            self.data = Format.get_formated_data(Format.load_json(path) ,sorted_time=True)

    def savaData(self, path):
        Format.save_json(self.data, path)
